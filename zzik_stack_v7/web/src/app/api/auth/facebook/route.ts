  ...
  try {
    ...
    if (loginType === 'instagram') {
      try {
        ...
        const pagesResult = await tfetch(
          `https://graph.facebook.com/v18.0/me/accounts?access_token=${tokenData.access_token}`,
          { timeout: 8000 }
        );
        
        if (pagesResult.status !== 'success' || !pagesResult.data) {
        ...
        if (pagesData.data && pagesData.data.length > 0) {
          ...
          if (igData.instagram_business_account) {
            ...
            const igProfileResult = await tfetch(
              `https://graph.facebook.com/v18.0/${igAccountId}?fields=id,username,profile_picture_url&access_token=${tokenData.access_token}`,
              { timeout: 8000 }
            );
            
            if (igProfileResult.status === 'success' && igProfileResult.data) {
              instagramInfo = igProfileResult.data;
            }
          }
        }
      } catch (igError) {
