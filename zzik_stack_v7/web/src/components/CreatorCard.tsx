...
export default function CreatorCard({ creator, onMatch, onViewDetail }: CreatorCardProps) {
  ...
  return (
    <article className="group relative p-6 bg-[var(--color-surface-elevated)] border border-[var(--color-border-primary)] rounded-xl hover:border-[var(--color-accent-primary)]/50 hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="absolute right-3 top-3">
        <MatchReasonTooltip creator={creator} />
      </div>
      ...
      <div className="space-y-3 pt-4 border-t border-[var(--color-border-primary)]">
        ...
        {onViewDetail && (
          <button
            onClick={() => onViewDetail(creator.id)}
            className="w-full h-9 text-xs font-medium text-[var(--color-text-secondary)] border border-[var(--color-border-primary)] rounded-lg hover:border-[var(--color-accent-primary)] hover:text-[var(--color-accent-primary)] transition-colors duration-200"
          >
            상세 보기
          </button>
        )}
