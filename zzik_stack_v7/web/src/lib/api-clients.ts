...
export const facebookOAuth = {
  ...
  exchangeCodeForToken: async (code: string) => {
    ...
    if (result.status !== 'success' || !result.data) {
      throw new Error(result.error?.message || 'Failed to exchange Facebook code for token');
    }
  ...
  getUserInfo: async (accessToken: string) => {
    ...
    if (result.status !== 'success' || !result.data) {
      throw new Error(result.error?.message || 'Failed to get Facebook user info');
    }
...
export const instagramAPI = {
  ...
  getUserProfile: async () => {
    ...
    if (result.status !== 'success' || !result.data) {
      throw new Error(result.error?.message || 'Failed to get Instagram profile');
    }
  ...
  getMediaList: async (limit = 10) => {
    ...
    if (result.status !== 'success' || !result.data) {
      throw new Error(result.error?.message || 'Failed to get Instagram media');
    }
...
export const sendGridAPI = {
  ...
  sendEmail: async ({
    ...
    const result = await tfetch('https://api.sendgrid.com/v3/mail/send', {
      method: 'POST',
      headers: {
    ...
    if (result.status !== 'success') {
      throw new Error(`SendGrid API error: ${result.error?.message || 'Unknown error'}`);
    }
  ...
  sendTemplateEmail: async ({
    ...
    if (result.status !== 'success') {
      throw new Error(`SendGrid Template API error: ${result.error?.message || 'Unknown error'}`);
    }
...
export const rapidAPI = {
  ...
  getInstagramProfile: async (username: string) => {
    try {
      ...
      if (result.status !== 'success' || !result.data) {
        throw new Error(`Instagram API error: ${result.error?.message || result.status}`);
      }
  ...
  getTikTokProfile: async (username: string) => {
    try {
      ...
      if (result.status !== 'success' || !result.data) {
        throw new Error(`TikTok API error: ${result.error?.message || result.status}`);
      }
  ...
  searchInstagram: async (query: string, type: 'hashtag' | 'place' | 'user' = 'user') => {
    try {
      ...
      if (result.status !== 'success' || !result.data) {
        throw new Error(`Instagram search error: ${result.error?.message || result.status}`);
      }
  ...
  searchTikTok: async (query: string, type: 'user' | 'video' | 'hashtag' = 'user') => {
    try {
      ...
      if (result.status !== 'success' || !result.data) {
        throw new Error(`TikTok search error: ${result.error?.message || result.status}`);
      }
...
export const tiktokOAuth = {
  ...
  exchangeCodeForToken: async (code: string) => {
    const result = await tfetch('https://open.tiktokapis.com/v2/oauth/token/', {
      method: 'POST',
      headers: {
    ...
    if (result.status !== 'success' || !result.data) {
      throw new Error(result.error?.message || 'Failed to exchange TikTok code for token');
    }
  ...
  getUserInfo: async (accessToken: string) => {
    const result = await tfetch('https://open.tiktokapis.com/v2/user/info/?fields=open_id,union_id,avatar_url,display_name', {
      method: 'GET',
      headers: {
  ...
  refreshToken: async (refreshToken: string) => {
    ...
    if (result.status !== 'success' || !result.data) {
      throw new Error(result.error?.message || 'Failed to refresh TikTok token');
    }
